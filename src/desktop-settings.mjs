...
async function validateRequest(request, loaded, io) {
  ...
  if (loaded.paths.mirrorDirOverride && requestedMirror && existingMirror
    ...
    errors.push(validationError('mirrorDir', 'environment-override-active', 'The mirror folder is controlled by BRIGHTSPACE_SYNC_MIRROR_DIR.'));
  }
...
export async function saveDesktopSettings(request, { runtime = {}, fileSystem = {}, forceCopy = false, writeConfig } = {}) {
  ...
  if (!syncLock.acquired) {
    return settingsFailure('operation-active', 'Another Brightspace operation is running. Try again when it finishes.', 'operation');
  }
