...
const installerSource = await read('installer/windows/CourseMirror.iss');
...
assert.match(productInclude, /https:\/\/github\.com\/aryanramz\/coursemirror/);
assert.match(productInclude, /https:\/\/github\.com\/aryanramz\/coursemirror\/issues/);
assert.match(productInclude, /https:\/\/github\.com\/aryanramz\/coursemirror\/releases/);
...
assert.match(installerSource, /^PrivilegesRequiredOverridesAllowed=none$/m);
...
assert.match(workflow, /name: coursemirror-installer-development/);
...
if (process.platform === 'win32') {
  const temp = await fs.mkdtemp(path.join(os.tmpdir(), 'coursemirror-installer-foundation-'));
  try {
    ...
    await fs.rm(path.join(incompleteRoot, 'dist', 'CourseMirror', 'CourseMirror.exe'));
    ...
    assert.notEqual(result.code, 0);
    assert.match(result.output, /Portable bundle is incomplete; required file is missing: CourseMirror\.exe/);

    const privateDataRoot = path.join(temp, 'private data bundle repo');
    await createSyntheticRepository(privateDataRoot);
    await fs.writeFile(path.join(privateDataRoot, 'dist', 'CourseMirror', 'config.json'), '{}\n', 'utf8');
    ...
    await fs.mkdir(path.join(browserRoot, 'dist', 'CourseMirror', 'app', 'node_modules', 'playwright-core', '.local-browsers'), { recursive: true });
