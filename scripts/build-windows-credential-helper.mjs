...
const PROJECT_DIR = path.join(ROOT, 'desktop', 'BrightspaceSync.CredentialHelper');
const OUTPUT_DIR = path.join(PROJECT_DIR, 'bin', 'Release');
const OUTPUT_EXE = path.join(OUTPUT_DIR, 'Brightspace Sync Credential Helper.exe');
...
async function build() {
  ...
  for (const [file, label] of [
    [path.join(PROJECT_DIR, 'BrightspaceSync.CredentialHelper.csproj'), 'credential-helper project'],
