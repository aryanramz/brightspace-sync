# Synthetic sample mirror

This directory shows the shape of Brightspace Sync output without containing any real student, course, instructor, or institution data.

The sample is intentionally small. A real mirror can contain additional section snapshots, HTML diagnostics, downloaded files, discussion details, content modules, term indexes, and system metadata depending on configuration and sync mode.

```text
sample-mirror/
├── 2026-Fall/
│   └── CSC 210 - Systems Programming [123456]/
│       ├── _course.json
│       ├── _course_status.json
│       ├── _latest_changes.json
│       └── assignments/
│           └── page.txt
└── _school/
    └── current.json
```

All names, IDs, dates, URLs, assignments, and values in this directory are fictional.
